...
- LLM access/setup - (OpenRouter via the OpenAI SDK, or BLOOM)

### Configuring OpenRouter access
- Export your API key: `export OPENROUTER_API_KEY=sk-...`
- (Optional) Override the base URL with `OPENROUTER_BASE_URL`.
- (Optional) Provide metadata headers expected by OpenRouter with
  `OPENROUTER_HTTP_REFERER` and `OPENROUTER_TITLE`.
- The toolkit now issues requests via the OpenAI **Responses API**, which is
  OpenAI's recommended interface and fully supported by OpenRouter. Chat
  engines should keep the `_chat` suffix so the pipeline prepares the right
  conversation payload for the Responses endpoint.

Example invocation once the environment is configured:
```
python3 llm_plan_pipeline.py --task t1 --config <config-name> \
    --engine anthropic/claude-3.5-sonnet_chat \
    --translator_engine anthropic/claude-3.5-sonnet
```
...
7. --translator_engine: The name of the engine to use for translation. Default is openai/gpt-4o. Pass the OpenRouter model identifier (append `_chat` for chat endpoints). Support for non-OpenRouter translation engines is not yet implemented.
...
- OpenRouter models:
  - For completion models just specify the OpenRouter model identifier.
    - openai/gpt-3.5-turbo-instruct, openai/gpt-4o-mini etc.
  - For chat models add the suffix '_chat' to the model name so the pipeline uses the chat endpoint.
    - anthropic/claude-3.5-sonnet_chat, mistralai/mistral-large_chat etc.
  - For fine-tuned models proxied through OpenRouter add the prefix 'finetuned:' to the model name
      - finetuned:openai/davinci:2022-05-03-00-00-00 etc.
